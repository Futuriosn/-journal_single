...
namespace kungfu {
    ...
    namespace yijinjing {
        ...
        namespace journal {
            ...
            writer::writer(const data::location_ptr &location, uint32_t dest_id, bool lazy) :
                    size_to_write_(0) {
            ...
            frame_ptr writer::open_frame(int64_t trigger_time, int32_t msg_type, uint32_t data_length) {
                assert(sizeof(frame_header) + data_length + sizeof(frame_header) <=
                       journal_->current_page_->get_page_size());
                ...
                if (journal_->current_frame()->address() + sizeof(frame_header) + data_length >
                    journal_->current_page_->address_border()) {
...
//                publisher_->notify();
            ...
            void writer::mark_with_time(int64_t gen_time, int32_t msg_type) {
                ...
                if (journal_->current_frame()->address() + sizeof(frame_header) >
                    journal_->current_page_->address_border()) {
